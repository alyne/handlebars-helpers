...
describe('assemble', function () {
  describe('absolute', function () {
    it('should create an absolute file path', function () {
      assert.equal(
        hbs.compile('{{absolute "a/b/c/package.json"}}')(),
        path.resolve('a/b/c/package.json')
      );
      assert.equal(
        hbs.compile('{{absolute "a/b/c/docs/toc.md"}}')(),
        path.resolve('a/b/c/docs/toc.md')
      );
    });

    it('should use the cwd on locals', function () {
      assert.equal(
        hbs.compile('{{absolute "a/b/c/package.json"}}')({ cwd: os.homedir() }),
        path.resolve(os.homedir(), 'a/b/c/package.json')
      );
      assert.equal(
        hbs.compile('{{absolute "a/b/c/docs/toc.md"}}')({ cwd: gm }),
        path.resolve(gm, 'a/b/c/docs/toc.md')
      );
    });
  });
  ...
  describe('dirname', function () {
    it('should get the dirname of a file path', function () {
      ...
      assert.equal(
        hbs.compile('{{dirname "a/b/c/docs/toc.md"}}')(),
        'a/b/c/docs'
      );
    });
  });
  ...
  describe('relative', function () {
    ...
    it('should return the relative path from file A to file B', function () {
      var fn = hbs.compile(
        '{{relative "examples/result/md/path.md" "examples/assets"}}'
      );
  ...
  describe('basename', function () {
    it('should get the basename of a file path', function () {
      assert.equal(
        hbs.compile('{{basename "a/b/c/package.json"}}')(),
        'package.json'
      );
