...
describe('comparison', function () {
  ...
  describe('compare', function () {
    ...
    describe('operators', function () {
      ...
      describe('>', function () {
        var fn = hbs.compile(
          '{{#compare a ">" b}}greater than or equal to 15{{else}}less than 15{{/compare}}'
        );
      ...
      describe('<', function () {
        var fn = hbs.compile(
          'I knew it, {{#compare unicorns "<" ponies}}unicorns are just low-quality ponies!{{else}}unicorns are special!{{/compare}}'
        );
      ...
      describe('<=', function () {
        var fn = hbs.compile(
          '{{#compare a "<=" b}}less than or equal to 10{{else}}greater than 10{{/compare}}'
        );
      ...
      describe('typeof', function () {
        it('should render the first block if true', function () {
          var fn = hbs.compile(
            '{{#compare obj "typeof" "object"}}A{{else}}B{{/compare}}'
          );
          assert(fn({ obj: {} }), 'A');
        });
      });
  ...
  describe('contains', function () {
    ...
    it('should render the inverse block if false.', function () {
      var fn = hbs.compile(
        '{{#contains context "zzz"}}A{{else}}B{{/contains}}'
      );
    ...
    it('should render the inverse block when an undefined argument is passed:', function () {
      var fn = hbs.compile(
        '{{#contains array nothing}}A{{else}}B{{/contains}}'
      );
      assert.equal(fn({ array: ['a', 'b', 'c'] }), 'B');
    });
  });
  ...
  describe('ifNth', function () {
    it('should render a custom class on even rows', function () {
      var source =
        '{{#each items}}<div{{#ifNth 2 @index}}{{else}} class="row-alternate"{{/ifNth}}>{{name}}</div>{{/each}}';
  ...
  describe('or', function () {
    describe('block', function () {
      ...
      it('should render a block if any of the values are truthy.', function () {
        var fn = hbs.compile(
          '{{#or great magnificent fantastic}}A{{else}}B{{/or}}'
        );
        assert.equal(
          fn({ great: false, magnificent: false, fantastic: true }),
          'A'
        );
      });
      it('should render the inverse block if neither are true.', function () {
      ...
      it('should render the inverse block if none are true.', function () {
        ...
        assert.equal(
          fn({ great: false, magnificent: false, fantastic: false }),
          'B'
        );
      });
    });
